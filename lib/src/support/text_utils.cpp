...
namespace cycfi { namespace elements
{
   ...
   namespace detail
   {
      char const* codepoint_to_utf8(unsigned cp, char str[8])
      {
         ...
         switch (n)
         {
            case 6: str[5] = 0x80 | (cp & 0x3f); cp = cp >> 6; cp |= 0x4000000; // fall through
            case 5: str[4] = 0x80 | (cp & 0x3f); cp = cp >> 6; cp |= 0x200000;  // fall through
            case 4: str[3] = 0x80 | (cp & 0x3f); cp = cp >> 6; cp |= 0x10000;   // fall through
            case 3: str[2] = 0x80 | (cp & 0x3f); cp = cp >> 6; cp |= 0x800;     // fall through
            case 2: str[1] = 0x80 | (cp & 0x3f); cp = cp >> 6; cp |= 0xc0;      // fall through
            case 1: str[0] = cp;
         }
         return str;
      }
